...
class Damage_manager():
    ...
    def take_damage(self, damage, source=None):
        ...
        context = Damage_context(damage, source)

        for modifier in self._sorted_modifiers:
            modifier.take_damage(context)
            if context.cancelled: return False                

        self.entity.apply_damage(context.damage)
        return True
...
class Tjasolmais_embrace(Modifier):#appended when tjasolmais embrace is equipped
    ...
    def take_damage(self, context):        
        self.entity.abilities.spirit_abilities['Shield'].shield.take_damage(context.damage)   #shieidl takes damage instead of entity     
