...
void main() {
    ...
    vec2 uv = fragmentTexCoord + speed * TIME * 0.1*fragmentTexCoord.y + 0.1*fragmentTexCoord.y*scroll/vec2(640,360);
    // Sample the noise texture
    float noise = texture(noise_texture, uv).r;
    // Convert the noise from the (0.0, 1.0) range to the (-1.0, 1.0) range
    // and clamp it between 0.0 and 1.0 again
    float fog = clamp(fragmentTexCoord.y*noise * 2.0 - 0.3, 0.0, 1.0);
