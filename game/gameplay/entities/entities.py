...
from gameplay.particles import particles
from engine.system import animation
from gameplay.narrative import dialogue
from gameplay.entities.player import backpack
from gameplay.entities.modifiers import modifier_damage, modifier_movement
from gameplay.entities.projectiles import seeds, hit_effects
from gameplay.entities.enemies import task_manager
from engine import constants as C

from gameplay.entities.base.entities_base import Enemy, Flying_enemy, NPC, Boss, Projectiles, Melee, Loot, Enemy_drop, Interactable, Interactable_item
from gameplay.entities.base.entities_core import Staticentity, Animatedentity, Platform_entity, Character

#from folder
from gameplay.entities.player import entity_shader_manager
from gameplay.entities.states import loot_container_states, runestone_states, player_states, packun_states, hitstop_states, states_savepoint, states_mygga_crystal, states_crab_crystal, states_exploding_mygga, states_droplets, states_twoD_liquid, states_death, states_lever, states_grind, states_portal, states_froggy, states_sword, states_fireplace, states_shader_guide, states_butterfly, states_cocoon_boss, states_maggot, states_horn_vines, states_camerastop, states_traps, states_NPC, states_enemy, states_vatt, states_enemy_flying, reindeer_states, states_bird, states_kusa, states_rogue_cultist, states_sandrew, states_blur, states_basic, rav_states, larv_wall_states, states_shader
...
class Reflection(Staticentity):#water, e.g. village
    ...
    def draw(self, target):
        ...
        screen_copy = self.game_objects.game.screen_manager.get_screen(layer = self.layer_name)
...
class Player(Character):
    def __init__(self, pos, game_objects):
        super().__init__(pos, game_objects)
        self.sounds = read_files.load_sounds_dict('assets/audio/sfx/enteties/aila/')
        self.sprites = read_files.load_sprites_dict('assets/sprites/enteties/aila/texture/', game_objects)
        self.normal_maps = read_files.load_sprites_dict('assets/sprites/enteties/aila/normal/', game_objects)
        self.image = self.sprites['idle'][0]
...
class Dash(Player_ability):
    def __init__(self,entity):
        ...
        self.sprites = read_files.load_sprites_dict('assets/sprites/ui/images/abilities/Dash/',entity.game_objects)
...
class Wall_glide(Player_ability):
    def __init__(self,entity):
        super().__init__(entity)
        self.sprites = read_files.load_sprites_dict('assets/sprites/ui/images/abilities/Wall_glide/',entity.game_objects)
        self.image = self.sprites['idle_1'][0]
...
class Double_jump(Player_ability):
    def __init__(self,entity):
        super().__init__(entity)
        self.sprites = read_files.load_sprites_dict('assets/sprites/ui/images/abilities/Double_jump/',entity.game_objects)
        self.image = self.sprites['idle_1'][0]
...
class Thunder(Projectiles):
    ...
    def pool(game_objects):
        Thunder.sprites = read_files.load_sprites_dict('assets/sprites/attack/Thunder/', game_objects)
...
class Sign(Interactable):
    def __init__(self,pos,game_objects,directions):
        ...
        self.sprites = read_files.load_sprites_dict('assets/sprites/animations/sign/',game_objects)
        self.image = self.sprites['idle'][0]
        self.rect = pygame.Rect(pos[0],pos[1],self.image.width,self.image.height)
        self.hitbox=self.rect.copy()
