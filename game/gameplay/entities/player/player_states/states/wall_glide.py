...
class WallGlide(PhaseBase):
    ...
    def enter(self, **kwarg):
        ...
        self.entity.flags[
            "grounddash"
        ] = True  # allow grounddash after wallglide, but not walljump, to give player more options out of wallglide
    ...
    def apply_wall_glide_profile(self, dt):
        profile = self.entity.contact_state.get_wall_glide_profile(
            side=self.get_wall_side()
        )
        if profile is None:
            self.fall()
            return
