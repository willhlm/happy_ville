...
class CrystalMus(Enemy):
    def __init__(self, pos, game_objects):
        super().__init__(pos, game_objects)
        self.config = ENEMY_CONFIG['base']
        self.sprites = read_files.load_sprites_dict('assets/sprites/entities/enemies/common/ground/crystal_mus/', game_objects, flip_x = True)
    ...
    def emit_tagg_burst(self):#perhaps make it so that easer to defeat with dash abilty as we can get close and go under the volley
        ...
        for angle_degrees in angles:
            ...
            projectile = CrystalTagg(spawn_pos, self.game_objects, lifetime=lifetime, velocity=velocity)
