...
class River(StaticEntity):#water, e.g. village
    def __init__(self, pos, game_objects, parallax, size, layer_name, **kwarg):
        ...
        self.reflect_rect0 = pygame.Rect(self.rect.left, self.rect.top, size[0], size[1]/self.squeeze)
        ...
        sounds = read_files.load_sounds_dict('assets/audio/sfx/entities/visuals/environments/river/')
    ...
    def draw(self, target):
        ...
        self.reflect_rect.bottomleft = [int(self.rect.topleft[0] - self.parallax[0]*self.game_objects.camera_manager.camera.interp_scroll[0]), int(-self.offset + self.rect.topleft[1] - self.parallax[1]*self.game_objects.camera_manager.camera.interp_scroll[1])]# the part to cut    
        self.reflect_rect0.bottomleft = [int(self.rect.topleft[0] - self.parallax[0]*self.game_objects.camera_manager.camera.interp_scroll[0]), int(-self.offset + self.rect.topleft[1] - (1-self.parallax[1])*self.game_objects.camera_manager.camera.interp_scroll[1])]#the pre factor (1-self.parallax[1]) should be 0 for parallax == 1

        blit_pos = (int(self.true_pos[0]-self.parallax[0]*self.game_objects.camera_manager.camera.interp_scroll[0]),int(self.true_pos[1]-self.parallax[1]*self.game_objects.camera_manager.camera.interp_scroll[1]))
        ...
        self.game_objects.shaders['water_perspective']['section0'] = [self.reflect_rect0[0],self.reflect_rect0[1],self.reflect_rect0[2],self.reflect_rect0[3]]

        #final rendering TODO: there is a small flickering of the reflected screen. This is cased by the camera stop, which isn't making it 100 % stationary. It makes the scroll go up and down by a pixel
        if self.parallax[0] == 1:#don't blur if there is no parallax            
            self.game_objects.game.display.render(self.empty.texture, target, position = blit_pos, section = self.reflect_rect0, scale = [1, self.squeeze], shader = self.game_objects.shaders['water_perspective'])
        else:
            ...
            self.game_objects.game.display.render(self.blur_layer.texture, target, position = blit_pos, section = self.reflect_rect0, scale = [1, self.squeeze], shader = self.game_objects.shaders['blur'])