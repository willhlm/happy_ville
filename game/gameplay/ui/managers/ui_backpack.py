import pygame, sys
from gameplay.ui.loaders import DomainMapLoader, InventoryLoader, JournalLoader, RadnaLoader, ScrollableWorldMapLoader
...
class MapUI(BaseUI):#local maps
    def __init__(self, game_objects, **kwarg):
        ...
        self.map_layer = game_objects.game.display.make_layer(game_objects.game.window_size)#te map
        self.mask_view = game_objects.game.display.make_layer(game_objects.game.window_size)#the darkend part
        self.mask = game_objects.game.display.make_layer(assumed_map_size)#assume same size. 
        self.mask_surface = pygame.Surface(assumed_map_size, pygame.SRCALPHA, 32).convert_alpha()
        self.has_mask = False
    ...
    def render(self):        
        ...
        self.game_objects.game.display.render(self.map_UI.BG, self.map_layer, position = self.pos)
        ...
        if self.has_mask:
            ...
            self.game_objects.game.display.render(self.mask.texture, self.mask_view, position=self.pos, shader = self.game_objects.shaders['blur'])
            shader = self.game_objects.shaders['mask']
            shader['maskTexture'] = self.mask_view.texture
            shader['maskColour'] = (0.0, 0.0, 0.0, 1.0)
            self.game_objects.game.display.render(self.map_layer.texture, self.game_objects.ui.screen, shader=shader)
        else:
            self.game_objects.game.display.render(self.map_layer.texture, self.game_objects.ui.screen)

        self.render_map_objects(self.get_map_ui_elements(), target=self.game_objects.ui.screen)
        self.render_map_objects(self.get_map_ui_buttons(), target=self.game_objects.ui.screen)
        self.blit_screen()
    ...
    def find_closest_position(self, direction):
        ...
        for container in self.map_UI.markers:
            if container == self.selected_container:
                continue
            target = container.rect
    ...
    def rebuild_mask(self):
        ...
        for area_id in revealed:
            for points in reveal_areas.get(area_id, []):
                if len(points) >= 3:
                    pygame.draw.polygon(self.mask_surface, (0, 0, 0, 0), points)

        mask_texture = self.game_objects.game.display.surface_to_texture(self.mask_surface)
    ...
    def render_map_objects(self, objects, target):
        for object in objects:
            if hasattr(object, 'draw'):
                ...
                self.game_objects.game.display.render(object.image, target, position = object.rect.topleft)
