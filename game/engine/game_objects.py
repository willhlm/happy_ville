...
from engine.system import (
    ...
    sequence_manager,
    deferred_texture_manager,
    map_resource_registry,
    stimuli,
)
from engine import controller
from engine.system.input_manager import InputManager
...
from gameplay.world.transforms.world_transform_controller import (
    WorldTransformController,
)
...
class GameObjects:
    def __init__(self, game):
        ...
        self.shaders = read_files.load_shaders_dict(
            self
        )  # load all shaders aavilable into a dict
        self.fade = FadeService(self)
        self.normal_map_generator = NormalMapGenerator(self)
        self.controller = controller.Controller()
        self.input_manager = InputManager(self.controller)
        ...
        self.sound = game_audio.GameAudio(
            camera_scroll_getter=lambda: self.camera_manager.camera.true_scroll
        )
        self.world_state = world_state.World_state(
            self
        )  # save/handle all world state stuff here
        self.world_controller = WorldController(self)
        self.ui = ui.UiManager(self)
        self.save_load = save_load.Save_load(
            self
        )  # contains save and load attributes to load and save game
    ...
    def _create_groups(self):  # define all sprite groups
        ...
        self.bg_interact = (
            groups.Group()
        )  # small grass stuff so that interactables blends with BG
        self.bg_fade = (
            groups.Group()
        )  # fg stuff that should dissapear when player comes: this should not blit or update. it will just run collision checks
        self.projectiles = groups.ProjectileGroups()
        self.loot = (
            groups.Group()
        )  # enemy drops and things player cn pickup upon collision
        self.entity_pause = groups.PauseGroup()  # all Entities that are far away
        self.cosmetics = (
            groups.Group()
        )  # things we just want to blit after the player layer
        self.cosmetics_bg = (
            groups.Group()
        )  # things we just want to blit before player layer
        self.interactables_fg = (
            groups.Group()
        )  # interacrables but are blitted after the player layer

        self.camera_blocks = groups.Group()  # camera blocks
        self.interactables = (
            groups.Group()
        )  # player collisions, when pressing T/Y and projectile collisions: chest, bushes, collision path, sign post, save point
        self.layer_pause = (
            groups.PauseLayer()
        )  # like eneitty pause but for those at different parallax layers
    ...
    def update(self, dt):
        ...
        self.entity_pause.update(
            dt
        )  # should be before enemies, npcs and interactable groups
    ...
    def update_render(self, dt):  # called after update_physics
        ...
        self.sound.update_render(
            self.player.hitbox.center
        )  # for emitters and tracking distance for sound
    ...
    def draw(self):  # called from render states
        ...
        self.interactables_fg.draw(
            player_fg_screen
        )  # shoud be after the player -> upstream, 2D water
    ...
    def _draw_hitboxes(self, target):
        ...
        if self.game.RENDER_HITBOX_FLAG:
            image = pygame.Surface(
                self.game.window_size, pygame.SRCALPHA, 32
            ).convert_alpha()
            ...
            pygame.draw.rect(
                ...
                (
                    round(
                        ...
                        - self.camera_manager.camera.true_scroll[0]
                    ),
                    round(
                        self.player.hitbox[1]
                        - self.camera_manager.camera.true_scroll[1]
                    ),
                    self.player.hitbox[2],
            ...
            pygame.draw.rect(
                ...
                (
                    round(
                        self.player.rect[0] - self.camera_manager.camera.true_scroll[0]
                    ),
                    round(
                        self.player.rect[1] - self.camera_manager.camera.true_scroll[1]
                    ),
                    self.player.rect[2],
                    self.player.rect[3],
                ),
            ...
            for (
                projectile
            ) in self.projectiles.friendly.sprites():  # go through the group
                pygame.draw.rect(
                    image,
                    (0, 0, 255),
                    (
                        int(
                            projectile.hitbox[0] - self.camera_manager.camera.scroll[0]
                        ),
                        int(
                            projectile.hitbox[1] - self.camera_manager.camera.scroll[1]
                        ),
                        projectile.hitbox[2],
                        projectile.hitbox[3],
                    ),
            ...
            for (
                projectile
            ) in self.projectiles.friendly_platform.sprites():  # go through the group
                pygame.draw.rect(
                    image,
                    (0, 0, 255),
                    (
            ...
            for (
                projectile
            ) in self.projectiles.enemy_platform.sprites():  # go through the group
                pygame.draw.rect(
                    image,
                    (0, 0, 255),
                    (
            ...
            for enemy in self.enemies.sprites():  # go through the group
                pygame.draw.rect(
                    ...
                    (0, 0, 255),
                    [
                        enemy.hitbox[0] - self.camera_manager.camera.scroll[0],
                        enemy.hitbox[1] - self.camera_manager.camera.scroll[1],
                        enemy.hitbox[2],
                        enemy.hitbox[3],
                    ],
                    2,
                ...
                pygame.draw.rect(
                    ...
                    (255, 0, 255),
                    [
                        enemy.rect[0] - self.camera_manager.camera.scroll[0],
                        enemy.rect[1] - self.camera_manager.camera.scroll[1],
                        enemy.rect[2],
                        enemy.rect[3],
                    ],
                    2,
            ...
            for cos in self.interactables.sprites():  # go through the group
                pygame.draw.rect(
                    ...
                    (0, 0, 255),
                    (
                        int(cos.hitbox[0] - self.camera_manager.camera.scroll[0]),
                        int(cos.hitbox[1] - self.camera_manager.camera.scroll[1]),
                        cos.hitbox[2],
                        cos.hitbox[3],
                    ),
                    1,
                ...
                pygame.draw.rect(
                    ...
                    (255, 0, 255),
                    (
                        int(cos.rect[0] - self.camera_manager.camera.scroll[0]),
                        int(cos.rect[1] - self.camera_manager.camera.scroll[1]),
                        cos.rect[2],
                        cos.rect[3],
                    ),
                    1,
                ...
                if type(platform).__name__ == "OneWayUpPlatform":
                    pygame.draw.rect(
                        ...
                        (
                            int(
                                platform.hitbox[0]
                                - self.camera_manager.camera.scroll[0]
                            ),
                            int(
                                platform.hitbox[1]
                                - self.camera_manager.camera.scroll[1]
                            ),
                            platform.hitbox[2],
                            platform.hitbox[3],
                        ),
            ...
            for fade in self.bg_fade:
                pygame.draw.rect(
                    ...
                    (
                        int(
                            fade.hitbox[0]
                            - fade.parallax[0] * self.camera_manager.camera.scroll[0]
                        ),
                        int(
                            fade.hitbox[1]
                            - fade.parallax[1] * self.camera_manager.camera.scroll[1]
                        ),
                        fade.hitbox[2],
                        fade.hitbox[3],
                    ),
            ...
            for light in self.lights.iter_lights():
                pygame.draw.rect(
                    ...
                    (
                        int(
                            light.hitbox[0]
                            - light.parallax[0] * self.camera_manager.camera.scroll[0]
                        ),
                        int(
                            light.hitbox[1]
                            - light.parallax[1] * self.camera_manager.camera.scroll[1]
                        ),
                        light.hitbox[2],
                        light.hitbox[3],
                    ),
                ...
                for obj in group:
                    if type(obj).__name__ == "River":
                        pygame.draw.rect(
                            ...
                            (0, 0, 255),
                            (
                                int(obj.reflect_rect[0]),
                                int(obj.reflect_rect[1]),
                                obj.reflect_rect[2],
                                obj.reflect_rect[3],
                            ),
                            1,
            ...
            for reflect in self.cosmetics:
                if type(reflect).__name__ == "River":
                    pygame.draw.rect(
                        ...
                        (0, 0, 255),
                        (
                            int(reflect.reflect_rect[0]),
                            int(reflect.reflect_rect[1]),
                            reflect.reflect_rect[2],
                            reflect.reflect_rect[3],
                        ),
                        1,
                    )  # draw hitbox
                    pygame.draw.rect(
