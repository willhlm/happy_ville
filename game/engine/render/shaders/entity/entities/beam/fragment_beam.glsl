...
uniform int roughness = 3; // How compact the noise texture will be
uniform int frequency = 10; // Amount of "ripples" in the beams

uniform float speed = 1.0; // Animation speed
uniform float thickness  = 0.006; // Thickness of the main beam
uniform float outline_thickness = 0.03; //Thickness of the outline color
uniform float beam_difference = 0.0; // The thickness difference between the main beam and the other, if there are more than one beam. The closer to 1 the smaller the thickness difference.

uniform float glow  = 0.0; // Use together with WorldEnvironment's Glow feature
...
uniform float y_offset  = 0.0; // Position of the beam
uniform float fixed_edge_size = 0.05; // How close to the edge should the beam be still before the animatino starts
uniform vec2 noise_scale = vec2(1); // If the object (for example the ColorRect or Sprite node) is compressed use this to compensate for the noise texture being compressed.
