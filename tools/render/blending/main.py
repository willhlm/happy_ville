...
while running:
    clock.tick()    
    engine.screen.clear(255,255,255,0)
    ref_screen.clear(255,255,255,0)#white to highlight the outlines
    time += 1
    ...
    if not direct:#render first to layer screens
        ...
        for screen in screens:#render results to ref screen at the end
            engine.set_premultiplied_alpha_blending()
            engine.render(screen.texture, ref_screen)
            engine.set_normal_alpha_blending()
