...
class RenderEngine:
    ...
    def __init__(self, screen_width: int, screen_height: int,
        ...
        # Read draw multiple object shader source files
        vertex_src = resources.read_text(
            'pygame_render', 'vertex_multiple.glsl')
        fragment_src_draw = resources.read_text(
            'pygame_render', 'fragment_multiple.glsl')

        # Create draw shader program
        prog_draw = self._ctx.program(vertex_shader=vertex_src,
                                      fragment_shader=fragment_src_draw)
        self._shader_draw_multiple= Shader(prog_draw)
    ...
    def render_objects(self,
        ...
        # Instance data: pos(x, y), scale(x, y), rotation, custom
        instance_data = []
        for inst in instances:
            pos = inst.get("pos", (0.0, 0.0))
            scale = inst.get("scale", (1.0, 1.0))
            rot = inst.get("rotation", 0.0)
            custom = inst.get("custom", 0.0)
            instance_data.append((*pos, *scale, rot, custom))
        instance_data = np.array(instance_data, dtype='f4')

        # Create buffers
        vbo_quad = self._ctx.buffer(quad_vertices.tobytes())
        vbo_instance = self._ctx.buffer(instance_data.tobytes())

        # Create vertex array object with per-vertex and per-instance attributes
        vao = self._ctx.vertex_array(shader.program, [
            (vbo_quad, '2f', 'vertexPos'),
            (vbo_instance, '2f 2f 1f 1f/i', 'instPos', 'instScale', 'instRot', 'instCustom'),
        ])

        # Set framebuffer, texture and shader
        layer.framebuffer.use()
        tex.use()
        shader.bind_sampler2D_uniforms()
